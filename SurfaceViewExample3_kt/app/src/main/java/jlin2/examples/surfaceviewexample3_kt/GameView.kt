package jlin2.examples.surfaceviewexample3_kt
    ...
    override fun run() {
        ...
        while (mRunning) {
            ...
            if (mSurfaceHolder.surface.isValid) {
                ...
                // Fill the canvas with green and draw the bitmap.
                canvas.save()
                canvas.drawColor(Color.GREEN)
                canvas.drawBitmap(mBitmap!!, mBitmapX.toFloat(), mBitmapY.toFloat(), mPaint)

                // Add clipping region and fill rest of the canvas with green.
                ...
                canvas.drawBitmap(BitmapFactory.decodeResource(getResources(),R.drawable.map),0F,0F,null)
                ...
                if (x > mWinnerRect!!.left && x < mWinnerRect!!.right && y > mWinnerRect!!.top && y < mWinnerRect!!.bottom) {
                    ...
                    canvas.drawBitmap(mBitmap!!, mBitmapX.toFloat(), mBitmapY.toFloat(), mPaint)
                    canvas.drawText("Got it!", (mViewWidth / 10).toFloat(), (mViewHeight / 2).toFloat(), mPaint)
                    if(mediaPlayer == null){
                        mediaPlayer = MediaPlayer.create(mContext,R.raw.gotit)
                        mediaPlayer?.start()
                    }
